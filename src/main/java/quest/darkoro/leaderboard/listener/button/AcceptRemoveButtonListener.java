...
@SecondaryListener
public class AcceptRemoveButtonListener extends ListenerAdapter {
  ...
  public void onButtonInteraction(ButtonInteractionEvent e) {
    if (e.isAcknowledged() || !e.getButton().getId().equals("accept_remove")) {
      return;
    }
    ...
    if (!e.getGuild().retrieveMemberById(e.getUser().getId()).complete().getRoles()
        .contains(permitted)) {
      e.reply("You don't have permission to remove users from the %s leaderboard!".formatted(
