...
public interface BoardRepository extends JpaRepository<Board, UUID> {
  ...
  @Query("SELECT b FROM Board b WHERE b.guildId=:guildId AND b.name=:name AND b.pending=false AND b.shared=false")
  Optional<Board> getLocalEntryForRemoval(Long guildId, String name);

  @Query(value = "SELECT * FROM board WHERE guild_id=:guildId AND shared=0 AND pending=0 AND level < :maxLevel ORDER BY level DESC LIMIT :limit", nativeQuery = true)
  List<Board> findTopByGuildId(Long guildId, Integer limit, int maxLevel);

  @Query(value = "SELECT * FROM board WHERE shared=1 AND pending=0 AND level < :maxLevel ORDER BY level DESC LIMIT :limit", nativeQuery = true)
  List<Board> findTopAll(Integer limit, int maxLevel);

  @Query(value = "SELECT * FROM board WHERE guild_id=:guildId AND shared=0 AND level=:maxLevel AND pending=0", nativeQuery = true)
  List<Board> findMaxLevelByGuildId(Long guildId, int maxLevel);

  @Query(value = "SELECT * FROM board WHERE shared=1 AND level=:maxLevel AND pending=0", nativeQuery = true)
  List<Board> findMaxLevelAll(int maxLevel);
